...
> Terakhir diperbarui: 26 April 2026 (Session 7).
...
- **Chunked Strategy** (`LLM_PRIMARY=nvidia`):
  ...
  - Chunk size: `NIM_CHUNK_SIZE=12` (default, configurable via env)
  - Filter output: hanya kirim DC cards dengan live position ke Telegram
  - Result: ~26/35 cards valid, ~310s per run, **gratis** (free tier 40 req/min)
- **Models tested** (accessible): `meta/llama-4-maverick-17b-128e-instruct` ✅, `meta/llama-3.3-70b-instruct` ✅ (tapi timeout)
- **Models NOT accessible**: `nvidia/llama-3.1-nemotron-70b-instruct` ❌, `meta/codellama-70b` ❌
