...
> Terakhir diperbarui: 02 Mei 2026 (Session 12).
...
| AI Engine | **Claude Sonnet 4.6** (primary) + **NVIDIA NIM** (fallback) + Gemini Flash (fallback-2) via `@anthropic-ai/sdk` + `axios` + `@google/genai` |
...
| `src/renderers/TelegramRenderer.ts` | **[UPDATED]** Render DC cards + scanner cards ke Telegram HTML; `renderDecisionCardsToTelegram()` |
| `src/prompts/buildMonitoringPrompt.ts` | Prompt assembly untuk AI — **MR guard dihapus dari signal generation (Session 12)** |
...
| `data/kill_switch.json` | **[BARU]** Global kill-switch state — survive server restart; atomic write |
...
11. **DC-2 override WAJIB update reason** — saat `card.action_now.action` diganti parity action, `card.action_now.reason` juga harus diupdate agar tidak kontradiksi di Telegram card.
12. **NIM chunk size MAX 5** — `NIM_CHUNK_SIZE` tidak boleh > 5 untuk strategi hedging (1 pair = ~1.5 posisi → 5 symbols × 1.5 = ~7-8 accountPositions, aman untuk NIM free tier).
...
### Perubahan Selesai (Session 12 — 02 Mei 2026)

| ID | Status | Deskripsi |
|----|--------|-----------|
| FORCE-RUN-DEBUG | ✅ DONE | `GET /api/debug/last-force-run-parity-input` endpoint baru; capture decision cards sebelum/sesudah NIM filter; `lastForceRunParityInput` + timestamp var |
| REASONING-FIX | ✅ DONE | `parityReasoning` fallback ditambah ke kedua call site `handlePaperDecisionOutput`; fallback di fungsi pakai `rawResult.whyBlocked \|\| whyAllowed`; empty reasoning count → 0 |
| DC-2-REASON-SYNC | ✅ DONE | DC-2 always override reason+reasoning saat mismatch; `card.action_now.reasoning` field juga diupdate; console.log dengan 160-char preview |
| DC-2-SUMMARY-SYNC | ✅ DONE | `card.parity_summary` Indonesian card-specific (`Override parity: HOLD — EXIT_READY, LOCK_1TO1...`); sync ke `card.summary/note/action_now.summary/note` |
| REASON-INDONESIAN | ✅ DONE | Fallback `parityReasoning` string → Indonesian: `HOLD — Konteks: X, struktur: Y, trend: Z (4H: W).` (kedua call site, `replace_all`) |
| TELEGRAM-IFTHEN | ✅ DONE | If/Then hanya render jika ada content; HOLD fallback pakai supply/demand zone prices (⬆️/⬇️ format); hapus "Konteks/Alasan" redundan dari fallback |
| TELEGRAM-SUMMARY | ✅ DONE | ℹ️ line prefer `card.parity_summary` → `card.status_line`; `isPotentiallyContradictoryHoldText()` guard execution verbs vs HOLD action |
| LEVELS-EMPTY-FIX | ✅ DONE | `📐 Levels:` header hanya render jika ada supply/demand/pivot/stop_hedge content |
| NIM-SCANNER-MERGE | ✅ DONE | `mergedScannerSignals` accumulator; `toArray()` + `extractScannerSignalsFromParsed()` helper; scanner chunk route ke `mergedScannerSignals`; `analysisJson` inject `new_signals.signals+scanner_cards+source:'NIM_SCANNER_CHUNK'` |
| NIM-SCANNER-DEBUG | ✅ DONE | `[NIM-SCANNER-DEBUG]` response snippet log (1000 chars); per-field extractor log (ns_signals, decision_cards, dll); `chunkDisplaySymbols` untuk scanner chunk |
| SCANNER-RENDERER | ✅ DONE | TelegramRenderer PART 2: `isNimScannerChunk` flag; skip legacy `NEW SIGNALS FOUND`; compact `🔭 SCANNER (N pair)` render per card (symbol, action, status, reason, if/then abbrev) |
| SCANNER-DEDUP | ✅ DONE | Legacy `signals` block guarded `!isNimScannerChunk` — eliminasi double render scanner |
| MR-SIGNAL-GUARD-REMOVE | ✅ DONE | `buildMonitoringPrompt.ts`: MR HARD GUARD → MR WARNING informational; SECTION 3 ≥25% → tetap generate signal; scanner signal gate MR<99% dihapus; keputusan eksekusi di user |
...
| # | Task | Priority |
|---|------|----------|
| 2.1 | Global Kill-Switch (sekarang bisa di local file, tidak perlu Firestore) | P0 | ✅ DONE — `loadKillSwitch/syncKillSwitchToDisk` di localStore.ts; `data/kill_switch.json`; 2 guard point (paper engine + force run); API GET/POST `/api/kill-switch`; Telegram alert saat aktivasi |
...
| `loadKillSwitch()` | Load global kill-switch state dari disk saat startup |
| `syncKillSwitchToDisk(state)` | Atomic write kill_switch.json setelah setiap perubahan |
...
- **Session 02 Mei 2026 (Session 12)** — Force Run validation patches: debug endpoint `/api/debug/last-force-run-parity-input`, reasoning fallback (empty reasoning → 0), DC-2 reason/summary selalu sync saat mismatch. Telegram DC format konsistensi: ℹ️ prefer `parity_summary`, `📐 Levels:` hanya render jika ada content, HOLD If/Then pakai supply/demand zone price (⬆️/⬇️ format), fallback reason Indonesian. NIM scanner signals: `mergedScannerSignals` accumulator + `extractScannerSignalsFromParsed()` helper (6-field extractor), scanner chunk router, `[NIM-SCANNER-DEBUG]` per-field log. TelegramRenderer PART 2: `isNimScannerChunk` flag eliminating double-render, compact `🔭 SCANNER` block. MR signal guard dihapus dari `buildMonitoringPrompt.ts` — AI selalu generate signal, MR hanya informational warning, keputusan eksekusi di user. Live validation: 14/14 DC cards, 14 Telegram messages + 1 scanner, DC-TRACE 5 MATCH / 9 MISMATCH (NIM=REDUCE → parity=HOLD). TSC clean.
- **Session 01 Mei 2026 (Session 11)** — NIM 504 fix: `NIM_CHUNK_SIZE=12→5` di .env + revert model `deepseek-v4-flash→llama-4-maverick`. Added `[NIM-FALLBACK]` Claude fallback di catch block chunk gagal (server.ts). DC-2 reason fix: override kini juga update `card.action_now.reason` agar tidak kontradiksi action. TelegramRenderer: `fmtPrice()` helper → desimal harga wajar (max 8 desimal). Tested Mistral Large 3 (675B) → ❌ 504 juga. Finding: NIM free tier hanya support model ≤~20B (llama-4-maverick 17B = ceiling). TSC clean.
- **Session 01 Mei 2026 (Session 10)** — Global Kill-Switch DONE: `loadKillSwitch/syncKillSwitchToDisk` di localStore.ts + `data/kill_switch.json` + 2 guard point (paper engine loop + force run) + API GET/POST `/api/kill-switch` + Telegram alert saat aktivasi. NIM: DeepSeek extra_body support + timeout 600s + debug logging chunk symbols. TSC clean. Kill-switch dinonaktifkan via `/api/kill-switch` (endpoint benar; `/api/system/kill-switch` = 404).
- **Session 27 Apr 2026 (Session 9 PM)** — Production validation: llama-4-maverick ✅ 10/10 cards, 100% MATCH, STO+TRADOOR included, full Telegram delivery
- **Session 27 Apr 2026 (Session 8, Final)** — Model iteration: deepseek-ai/deepseek-v4-pro ❌ (504 timeout) → fallback ke meta/llama-4-maverick-17b-128e-instruct ✅ (STABLE)
- **Session 27 Apr 2026 (Session 8, Part 2)** — NVIDIA NIM nemotron-3 tuning: max_tokens → 32000, temperature → 0.1, explicit system prompt, debug logging
...
### Fallback 1: NVIDIA NIM (Chunked Strategy)

- Model: `meta/llama-4-maverick-17b-128e-instruct` (env: `NVIDIA_NIM_MODEL` di `.env`)
- API Key: `NVIDIA_NIM_API_KEY` di `.env`
- Base URL: `https://integrate.api.nvidia.com/v1`
- Max tokens per chunk: **16000**
- Timeout: **600 detik**
- **Chunked Strategy** (`LLM_PRIMARY=nvidia`):
  - Split symbols → `openPositionSymbols` (DC chunks) + `scannerUniverse` (scanner chunk terakhir)
  - Chunk size: `NIM_CHUNK_SIZE=5` (**Session 11 fix**: turun dari 12 — 12 symbols + 18 positions terlalu besar → 504)
  - Filter output: hanya kirim DC cards dengan live position ke Telegram
  - Result: ~14 cards valid, ~310s per run, **gratis** (free tier 40 req/min)
  - **[NIM-FALLBACK]** Jika chunk gagal 504 → otomatis fallback ke Claude untuk chunk tersebut (Session 11)
- **Production Result (Session 9, 27 Apr 2026 PM)**: ✅ 10/10 cards extracted, 100% DC-TRACE MATCH, STO+TRADOOR included, ~310s per run
- **Models tested**: `meta/llama-4-maverick-17b-128e-instruct` ✅ (PROD READY, ~17B = ceiling NIM free tier)
- **Models FAILED**: `deepseek-ai/deepseek-v4-pro` ❌ (504), `deepseek-ai/deepseek-v4-flash` ❌ (belum terbukti, diganti), `nvidia/nemotron-3-super-120b-a12b` ❌ (timeout), `nvidia/llama-3.1-nemotron-70b-instruct` ❌, `meta/codellama-70b` ❌, `mistralai/mistral-large-3-675b-instruct-2512` ❌ (504 — terlalu besar)
- **NIM Free Tier Rule**: Model > ~20B parameter → timeout. llama-4-maverick (17B) adalah batas aman.

### Fallback 2: Gemini Flash

- Model: `gemini-2.5-flash` (env: `GEMINI_MONITOR_MODEL` di `.env`)
- API Key: `GEMINI_API_KEY` di `.env`
- Hard cap: `maxOutputTokens: 8192`
- Aktif jika Claude + NIM gagal
- **Catatan:** gemini-1.5-flash deprecated; gemini-2.0-flash "not available to new users"; gemini-2.5-flash = thinking model → 0 cards parsed (format berbeda). Fallback terakhir saja.

### Fallback Chain

```
LLM_PRIMARY=claude  →  Claude → NIM → Gemini
LLM_PRIMARY=nvidia  →  NIM (chunked) → Claude → Gemini
LLM_PRIMARY=gemini  →  Gemini → Claude → NIM
```
...
| NVIDIA NIM Chunked (35 pairs, 3 chunks) | ~20K / ~16K per chunk | **~$0** (free tier) |
| Gemini 2.5 Flash (fallback) | ~67K / ~8K | ~$0.01 |
...
[x] Global Kill-Switch aktif ✅ DONE — GET/POST `/api/kill-switch`; 2 guard points; data/kill_switch.json
...
|---------|-----------|--------|
| 2026-04-26 | NVIDIA NIM Chunked Strategy DONE (Session 7) | Claude credit habis → explore NVIDIA NIM free tier. Model viable: `meta/llama-4-maverick-17b-128e-instruct`. Strategi: split 35 symbols → 2 DC chunks (openPositionSymbols) + 1 scanner chunk (scannerUniverse). max_tokens=16000 per chunk, repair truncated JSON. Filter: hanya DC cards dengan live position dikirim ke Telegram. Result: 26/28 cards valid, 308s per run, gratis. `LLM_PRIMARY=nvidia`, `NVIDIA_NIM_MODEL`, `NIM_CHUNK_SIZE=12` di .env. |
| 2026-04-26 | GEMINI_MONITOR_MODEL diupdate ke gemini-2.5-flash | gemini-1.5-flash + gemini-2.0-flash deprecated/not available. gemini-2.5-flash accessible tapi thinking model — 0 cards karena format berbeda. Fallback chain: Claude → Gemini → NVIDIA NIM. |
| 2026-04-26 | MAX_DRAWDOWN_PCT diperlebar ke 80% | Default 20% terlalu ketat untuk paper trading import live. Equity turun 50.5% dari peak → engine berhenti. Fix: MAX_DRAWDOWN_PCT=80 di .env + reset-drawdown-peak endpoint dipanggil ulang. |
...
| 2026-04-27 | BUG-NIM-TIMEOUT: nemotron-3 terlalu lambat (Session 8) | Chunk 1 TIMEOUT after 300s — fix: timeout → 600s, chunk_size → 6 (dari 12). Alternative: fallback ke llama-4-maverick yang lebih cepat |
| 2026-04-27 | BUG-NIM-CHUNK: nemotron-3 tuning (Session 8) | Model `nvidia/nemotron-3-super-120b-a12b` ACCESSIBLE tapi incomplete output — fix: max_tokens → 32000, temperature → 0.1, explicit system prompt, debug logging untuk symbol tracking |
| 2026-05-02 | MR SIGNAL GUARD REMOVED (Session 12) | `buildMonitoringPrompt.ts`: 4 lokasi diubah dari hard-block ke informational warning. MR HARD GUARD → MR WARNING; SECTION 3 ≥25% → "zona risiko tinggi, tetap generate signal"; "JANGAN sarankan ekspansi" → "tetap hasilkan signal + catatan MR"; scanner gate MR<99% dihapus. Desain: AI selalu generate signal terbaik, keputusan eksekusi di user. Parity engine (server.ts) tetap punya MR guard untuk execution. |
| 2026-05-02 | NIM SCANNER CHUNK DELIVERY (Session 12) | Root cause: `new_signals = { scanner_cards: [...] }` tapi TelegramRenderer PART 2 hanya baca `new_signals.signals`. Fix: (1) `mergedScannerSignals` accumulator + `extractScannerSignalsFromParsed()` 6-field extractor di server.ts; (2) scanner chunk routed ke `mergedScannerSignals` bukan `mergedCards`; (3) `analysisJson` inject `new_signals.signals+scanner_cards+source:'NIM_SCANNER_CHUNK'`; (4) TelegramRenderer: `isNimScannerChunk` flag skip legacy render, compact `🔭 SCANNER` block. Live: `ns_signals:1` found, 15 Telegram messages (14 DC + 1 scanner). |
| 2026-05-02 | TELEGRAM DC FORMAT CONSISTENCY (Session 12) | 5 format issues diperbaiki: (1) `📐 Levels:` kosong → hanya render jika ada content; (2) HOLD If/Then fallback pakai supply/demand zone prices (⬆️/⬇️ format, hapus Konteks/Alasan redundan); (3) ℹ️ line prefer `card.parity_summary` → guard execution verbs bertentangan HOLD; (4) `paritySummaryText` Indonesian card-specific context; (5) `parityReasoning` fallback Indonesian: `HOLD — Konteks: X, struktur: Y, trend: Z (4H: W).` |
| 2026-05-02 | FORCE RUN VALIDATION DEBUG (Session 12) | Empty reasoning 14/14 → 0: DC-PARITY pre-populate call site tidak pass `reasoning` field; `handlePaperDecisionOutput` tidak ada fallback. Fix: `parityReasoning` di kedua call site + `String(rawResult.reasoning ?? whyBlocked ?? whyAllowed ?? '') \|\| undefined` di fungsi. Endpoint baru `GET /api/debug/last-force-run-parity-input` — capture 18 fields setelah NIM filter, sebelum DC-TRACE. DC-2 reason sync diperkuat: always override + `card.action_now.reasoning` juga diupdate. |
| 2026-05-01 | NIM 504 FIXED + NIM-FALLBACK (Session 11) | Root cause: `NIM_CHUNK_SIZE=12` default terlalu besar — 12 symbols × 1.5 legs = 18 accountPositions → payload besar → NIM 504. Fix: `NIM_CHUNK_SIZE=5` di .env + revert model dari `deepseek-v4-flash` ke `llama-4-maverick`. Added `[NIM-FALLBACK]`: catch block chunk gagal → fallback ke Claude untuk chunk tersebut, symbols tidak lagi silent drop. Result: 14 DC terkirim. |
| 2026-05-01 | DC-2 REASON SYNC (Session 11) | DC-2 override ganti `card.action_now.action` tapi tidak update `reason` → Telegram card kontradiksi (action=HOLD tapi reason="Reduce short 0.5..."). Fix: saat `geminiAction !== parityAction`, update `card.action_now.reason` dengan `parityDecision.whyBlocked \|\| whyAllowed \|\| reasoning`. |
| 2026-05-01 | TelegramRenderer fmtPrice (Session 11) | Level harga di Telegram card tampil 16+ desimal (misal `0.07096284708139197`). Fix: `fmtPrice()` helper di `TelegramRenderer.ts` — rounding otomatis sesuai magnitude (≥1000→2dec, ≥1→4dec, ≥0.01→5dec, ≥0.001→6dec, else→8dec). Applied ke supply/demand/pivot/stop_hedge/if_then levels. |
| 2026-05-01 | Mistral Large 3 TESTED → FAILED (Session 11) | `mistralai/mistral-large-3-675b-instruct-2512` (675B MoE) → 504 timeout pada Chunk 1 (5 symbols, 7 positions). Konfirmasi pola: NIM free tier hanya support model ≤~20B parameter. Reverted ke `llama-4-maverick`. Kandidat belum ditest: `google/gemma-4-31b-it`, `nvidia/nemotron-3-nano-30b-a3b`. |
| 2026-05-01 | GLOBAL KILL-SWITCH DONE (Session 10) | `KillSwitchState` + `loadKillSwitch/syncKillSwitchToDisk` di localStore.ts. File `data/kill_switch.json` (atomic write). Guard Point 1: paper engine loop skip if enabled. Guard Point 2: force run 503 if enabled. API: GET/POST `/api/kill-switch`. Telegram alert saat aktivasi. NIM: DeepSeek extra_body `{thinking:false}` support + timeout 600s + debug logging chunk symbols/accountPositions. TSC clean. |
| 2026-04-27 | NVIDIA NIM Production Ready (Session 9 PM) | `meta/llama-4-maverick-17b-128e-instruct` validation: ✅ 10/10 cards extracted, 100% DC-TRACE MATCH (gemini=HOLD vs parity=HOLD), STO+TRADOOR included, ~310s/run, free tier 40 req/min, 10 Telegram messages sent — PROD READY |
